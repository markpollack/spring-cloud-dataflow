...
* Use the Spring Boot code formatter from https://raw.githubusercontent.com/spring-projects/spring-boot/master/eclipse/eclipse-code-formatter.xml[here]
* If you are using `IntelliJ` then Go to Preferences -> Editor -> Code Style -> Java -> Manage and import the formatter file

  ** Go to Import settings in Preferences -> Editor -> Code Style -> Java -> Import and set
  `Class count to use import with '\*'` and `Names count to use static import with '*'` to high values (say 999) to avoid using this property
  ** In the same page, set the Import order with the following settings
